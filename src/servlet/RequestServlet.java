...
public class RequestServlet extends HttpServlet {
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		// test10(request);
		// test11(request);
		//test12(request, response);
		response.getWriter().write("999999");
		RequestDispatcher rd = request.getRequestDispatcher("DispatcherServlet");
		rd.include(request, response);
	}

	private void test12(HttpServletRequest request, HttpServletResponse response)
			throws IOException, ServletException {
		//转发  配合DispatcherServlet中的test1方法使用
		response.setContentType("text/html;charset=utf-8");
		request.setAttribute("p", "pp");
		response.getWriter().write("放了");
//		ServletContext sc = this.getServletContext();
//		RequestDispatcher rd = sc
//				.getRequestDispatcher("/servlet/DispatcherServlet");
		RequestDispatcher rd = request.getRequestDispatcher("DispatcherServlet");
		rd.forward(request, response);
//		response.sendRedirect("/WebDemo/servlet/DispatcherServlet");
	}

	private void test11(HttpServletRequest request)
	...
	private void test8(HttpServletRequest request) {
		...
		for (Map.Entry<String, String[]> me : map.entrySet()) {
			...
			try {
				PropertyDescriptor pd = new PropertyDescriptor(paraName,
						User.class);
